...
一个带前端的重型 **Agent Skill**：把会前准备、会议记录、会后推进和项目脉络组织成一套可以持续工作的方式。让你自己的 Agent 接手，按你的资料、权限与工作习惯搭建。

> 开源的是工作方法、前端源码、数据契约和连接器参考。首次使用需要 Agent 做适配与验证；这里没有托管服务、预连账号或完整的自托管后端。

[项目海报](docs/images/huili-poster.png) · [交给 Agent 接手](docs/TAKEOVER.md) · [Skill 入口](SKILL.md) · [能力与边界](docs/CAPABILITIES.md) · [接入契约](docs/ADAPTER.md) · [隐私说明](docs/PRIVACY.md)
...
```text
请阅读这个仓库的 SKILL.md 和 docs/TAKEOVER.md。
我想基于会里搭建自己的会议与工作推进台。请先核对你具备的工具、
我授权的资料范围和部署条件，给出最小接入方案。
保留前端的整体设计语言，用我的账号与数据；先完成一场会议的
读取、准备、行动确认、产物回读，再按需要扩展项目和随手记。
缺失的条件请明确标记，先不要开启后台任务或把业务状态标为完成。
```
...
| 目录 | 交付内容 |
| --- | --- |
| `SKILL.md`、`references/`、`templates/` | 接手协议、六条工作流、默认模板和验收条件 |
...
除了会议日历、会前准备、会后行动和系列会议，已纳入较新的项目责任视图、随手记完成与撤销、背景交接，以及今日留白的开放表达与丢弃机制。开源前端展示与原运行环境的上线状态分开说明，详见[能力与边界](docs/CAPABILITIES.md)。

## 隐私与许可

发布副本采用文件白名单和全新 Git 历史。移除作者个人画像、姓名别名、行业默认分类、真实会议、聊天、项目目录、源工作区路径、内部链接、实例标识、凭证、运行状态和旧截图。示例没有从真实业务数据中抽样。[隐私说明](docs/PRIVACY.md)

项目自有代码与文档采用 [MIT License](LICENSE)。第三方依赖按各自许可使用，见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
