...
我把这种形态称为 **种子应用（Seed Apps）**；会里是其中一颗。它以 Skill 分发，由你的 Agent 在你的环境里生成或融入带前端的应用。包里有工作方法、审美和前端参考；你的 Agent 用本人授权的资料和少量问题理解你的工作。已有个人工作台，就在授权范围内融入需要的能力；还没有，就生成一个可以持续使用的起点。它把会前准备、会议记录、会后推进和项目脉络接到同一条工作线上。
...
**接收者不需要从作者的个人妙搭应用下载另一份连接包。** 取得本仓库后，让自己的 Agent 按 `SKILL.md` 在自己的环境里配置、部署与连接。首次使用仍需核对资料授权、运行环境、存储和验收；可选连接器仅服务于使用者已有兼容后端的情况。

[项目海报](docs/images/huili-poster-v4.png) · [首次生成个人会里](references/first-run.md) · [可选视觉定制](references/visual-customization.md) · [交给 Agent 接手](docs/TAKEOVER.md) · [Skill 入口](SKILL.md) · [能力与边界](docs/CAPABILITIES.md) · [接入契约](docs/ADAPTER.md) · [隐私说明](docs/PRIVACY.md) · [使用许可](docs/LICENSE-FAQ.md)
...
先问我是否已有常用的个人工作台：有的话，请确认项目位置和可读/可改范围，
在授权后了解原有风格、入口和数据组织，再提出最小的融入方案；没有的话，
从会里的参考前端开始。也问我想沿用哪种风格，允许我直接跳过。
...
已有工作台时延续它的设计语言；独立起点默认沿用会里视觉。
如果我给出参考页面或审美偏好，再按我的选择调整。
请在我的环境里配置、部署和连接；不要把作者个人妙搭应用的连接包当成前置条件。
...
打开 `http://127.0.0.1:8136/` 看“此刻”与留白抽屉，或打开 `http://127.0.0.1:8136/?view=projects` 看项目。示例业务内容与依据均为虚构；「句点暂留」保留个人版的视觉交互，「镜子各忙各的」保留清除元数据后的原图。留白反馈、抽屉轮换和随手记只在当前页面保留，刷新后重置。执行 Agent、联网写入与后台调度均未接入。[演示讲解说明](DEMO-README.md)
...
另补充了[首次生成个人会里](references/first-run.md)的两条路径、接手协议和验收步骤；它尚未纳入已发布版本，也没有证明其他使用者的 Agent 已能自动完成生成或融入任意工作台。
