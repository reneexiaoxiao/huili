...
本仓库未包含完整认证/数据库后端。前端可以先被借鉴；要持续使用，请由接手 Agent 在使用者自己的环境中实现这些边界。默认路径不依赖作者个人妙搭应用，不需要从其页面下载安装连接包。取得 Skill 仓库与配置本人环境仍是必要步骤；不要复制别人的实例、事件监听或配对配置。
...
只有使用者已有实现了下列 `/openapi/v1` 路径的自有兼容后端时，才考虑这个连接器。它延续个人版的配对协议，不是从 Skill 生成应用的前置步骤，也不是通用部署器。需要 **Python 3.10+**。此程序仅按需运行；不安装 CLI、不启动事件监听、不调用模型、不修改宿主配置。
